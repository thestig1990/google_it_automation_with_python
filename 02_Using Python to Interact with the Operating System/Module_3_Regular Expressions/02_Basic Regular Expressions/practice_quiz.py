...
# Question 2
# The check_time function checks for the time format of a 12-hour clock, as follows: the hour is between 1
# and 12, with no leading zero, followed by a colon, then minutes between 00 and 59, then an optional space,
# and then AM or PM, in upper or lower case. Fill in the regular expression to do that. How many of the
# concepts that you just learned can you use here?

import re
def check_time(text):
  pattern = "(^1[0-2]|[1-9]):([0-5][0-9])( ?([AaPp][Mm]))"
  result = re.search(pattern, text)
  return result

print(check_time("1:45pm")) # True
...
# is a set of communication technologies used for text-based communication" should return True 
# since (IM) satisfies the match conditions." Fill in the regular expression in this function:
...
# 1. Matches plus sign characters
# 2. Matches one or more occurrences of the character before it - "RIGHT!"
# 3. Matches the end of a string
# 4. Matches the character before the [+] only if there is more than one
